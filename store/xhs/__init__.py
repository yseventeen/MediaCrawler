...
class XhsStoreFactory:
    STORES = {
        "feishu": XhsFeishuStoreImplement,
    ...
    def create_store() -> AbstractStore:
        ...
        if not store_class:
            raise ValueError(
                "[XhsStoreFactory.create_store] Invalid save option, supported: feishu, csv, db, json, sqlite, mongodb, excel, postgres"
            )
        return store_class()
...
async def update_xhs_note(note_item: Dict):
    ...
    utils.logger.info(f"[store.xhs.update_xhs_note] note saved note_id={note_id}")
    await XhsStoreFactory.create_store().store_content(local_db_item)
    if config.SAVE_DATA_OPTION != "feishu":
        try:
            from feishu.sync import sync_xhs_content_to_feishu
            await sync_xhs_content_to_feishu(local_db_item)
        except Exception as e:
            utils.logger.warning(
                f"[store.xhs.update_xhs_note] Feishu contents 表同步失败（请检查 .env 中 FEISHU_BITABLE_TABLE_ID_CONTENTS 及飞书表列名）: {type(e).__name__}: {e}"
            )
...
async def update_xhs_note_comment(note_id: str, comment_item: Dict):
    ...
    utils.logger.info(f"[store.xhs.update_xhs_note_comment] comment saved comment_id={comment_id} note_id={note_id}")
    await XhsStoreFactory.create_store().store_comment(local_db_item)
    if config.SAVE_DATA_OPTION != "feishu":
        try:
            from feishu.sync import sync_xhs_comment_to_feishu
            await sync_xhs_comment_to_feishu(local_db_item)
        except Exception as e:
            utils.logger.warning(
                f"[store.xhs.update_xhs_note_comment] Feishu comments 表同步失败（请检查 .env 中 FEISHU_BITABLE_TABLE_ID_COMMENTS 及飞书表列名）: {type(e).__name__}: {e}"
            )
...
async def save_creator(user_id: str, creator: Dict):
    ...
    utils.logger.info(f"[store.xhs.save_creator] creator saved user_id={user_id}")
