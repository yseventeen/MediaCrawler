...
KEYWORDS = "穿搭,穿什么"  # 关键词搜索配置，以英文逗号分隔
...
# 数据保存类型选项配置：feishu(仅同步飞书)、csv、db、json、sqlite、excel、postgres
SAVE_DATA_OPTION = "feishu"  # feishu | csv | db | json | sqlite | excel | postgres
...
# 是否在搜索时同时抓取并保存「搜索推荐词」（搜索框下方约 10 个联想词，仅小红书）
ENABLE_GET_SEARCH_SUGGESTIONS = True
