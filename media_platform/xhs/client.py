...
class XiaoHongShuClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    def _parse_suggestion_item(item: Dict) -> Optional[str]:
        ...
        if not item:
            return None
        # 常见字段名：query, keyword, title, desc；有的在子对象里
        ...
        if isinstance(query_data, dict):
            for key in ("query", "keyword", "title", "desc"):
                val = query_data.get(key)
                if isinstance(val, str) and val.strip():
                    return val.strip()
        return None

    @staticmethod
    def parse_search_suggestions_from_response(search_response: Dict, keyword: str) -> Dict[str, Any]:
        """
        从「搜索笔记」接口的返回中解析出推荐词（不发起新请求）。
        推荐词来自 items 里 model_type 为 rec_query / hot_query 的条目。
        """
        rec_list: List[str] = []
        hot_list: List[str] = []
    ...
    async def get_search_suggestions(self, keyword: str) -> Dict[str, Any]:
        """
        获取小红书搜索推荐词（搜索框输入关键词后下方出现的约 10 个相关推荐词）。
        通过调用搜索笔记接口的第一页，从返回的 rec_query / hot_query 条目中解析得出。
        ...
        Returns:
            {
                "keyword": str,
                "suggestions": [str, ...],   # 所有推荐词（去重）
                "rec_query": [str, ...],     # 推荐联想词
                "hot_query": [str, ...],     # 热门搜索词
            }
        """
        res = await self.get_note_by_keyword(
            keyword=keyword,
            page=1,
            page_size=20,
        )
        ...
        for item in res.get("items") or []:
            model_type = item.get("model_type")
            text = self._parse_suggestion_item(item)
            if not text:
                continue
            if model_type == "rec_query":
                rec_list.append(text)
